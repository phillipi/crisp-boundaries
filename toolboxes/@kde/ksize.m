...
% Copyright (C) 2003 Alexander Ihler; distributable under GPL -- see README.txt
  ...
  case {'lcv2'},
    [minm,maxm] = neighborMinMax(npd);
    npd = kde(getPoints(npd),(minm+maxm)/2,getWeights(npd),getType(npd));
    ks =  golden(npd,@nLOO_LL2,2*minm/(minm+maxm),1,2*maxm/(minm+maxm),1e-2);
    ks = ks * (minm+maxm)/2;
  
  case {'lcv_anis'},
    %[minm,maxm] = neighborMinMax(npd);
    %npd = kde(getPoints(npd),(minm+maxm)/2,getWeights(npd),getType(npd));
    %ks =  golden(npd,@nLOO_LL_anis,2*minm/(minm+maxm),1,2*maxm/(minm+maxm),1e-2);
    
    %{
    %% first solve for bandwidth in each dimension independently
    ks = [];
    for i=1:(getDim(npd)/2)
        npd_tmp = marginal(npd,i);
        [minm,maxm] = neighborMinMax(npd_tmp);
        npd_tmp = kde(getPoints(npd_tmp),(minm+maxm)/2,getWeights(npd_tmp),getType(npd_tmp));
        ks(i) =  golden(npd_tmp,@nLOO_LL,2*minm/(minm+maxm),1,2*maxm/(minm+maxm),1e-2);
        ks(i) = ks(i) * (minm+maxm)/2;
    end
    %ks = cat(2,ks,ks);
    ks = ks';
    %}
    
    npd_tmp = ksize(npd,'lcv');
    ks = getBW(npd_tmp,1);
    ks = ks(1:3);
    ...
    %% now solve jointly
    f = @(alpha,p) nLOO_LL_anis(alpha,npd);
    %fminsearch_opts.Display = 'iter';
    ks = fminsearch(@(alpha) f(alpha,npd), ks);%, fminsearch_opts);%ones(getDim(npd)/2,1),fminsearch_opts);
    %ks = ks * (minm+maxm)/2;
    ks = cat(1,ks,ks);
  ...
  case {'rot'},                     % "Rule of Thumb" (stddev-based)
    ks = ksizeROT(npd); %ks([1,4]) = 0.01;
  case {'rot_reg'},
    ks = ksizeROT(npd);
    lambda = 0.02;
    ks = (1-lambda)*ks+lambda;
...
function H = nLOO_LL2(alpha,npd)
  ...
  %H = entropy(npd,'lvout');
  Hjoint = entropy(npd,'lvout');
  nDims = getDim(npd);
  p_marg = marginal(npd,(nDims/2)+1:nDims);
  Hmarg = entropy(p_marg,'lvout');
  Hconditional = Hjoint - Hmarg;
  H = Hconditional;
...
function H = nLOO_LL_anis(alpha,npd)
  %if (nargin < 2) error('ksize: LOO_LL: Error!  Too few arguments'); end;
  %if (npd.type == 0) alpha = alpha.^2; end;
  alpha = cat(1,alpha,alpha);
  alpha = repmat(alpha,[1,size(npd.bandwidth,2)]);
  npd.bandwidth = npd.bandwidth .* alpha;
  H = entropy(npd,'lvout');
  npd.bandwidth = npd.bandwidth ./ alpha;
