...
function ae_mx = ae_subproblem_fold(ae_prob, tol_ichol, transform_flag)
   ...
   for s = nlvls:-1:1
      ...
      if (isempty(U))
         ...
         R = ichol(U' * U, struct('type','ict','droptol',tol_ichol,'shape','upper'));
      ...
      if (s > 1)
         ...
         Rb_arr{s} = ichol(Ub' * Ub, struct('type','ict','droptol',tol_ichol,'shape','upper'));
         ...
         if (transform_flag)
            ...
            Ra = ichol(Ua' * Ua, struct('type','ict','droptol',tol_ichol,'shape','upper'));
